...
function index() {
  ...
  const { ParticpantFiltersData, setSelectedTableRows, setSelectedRowObjects, selectedTableRows, selectedRowObjects } =
    ParticipantStore()
  const filters: {
    /**
  ...
  const handleUpdateAttendanceStatus = async (attendance_status_id: number) => {
    ...
    if (!error) {
      ...
      setBulkActionsErrorTitle(`${participantIds?.length} Records Successfully Updated`)
  ...
  const handleBulkUpdateTransactionStatus = async (transaction_status_id: number) => {
    ...
    const selectTransactionStatusIds = selectedTransactionStatusValues?.map(record => record?.transaction_status_id)

    const allValuesSame = (arr: any): boolean => arr.every((val: any) => val === transactionPendingStatusID)
    ...
    if (selectTransactionStatusIds?.length) {
      if (allValuesSame(selectTransactionStatusIds)) {
        ...
        if (!error) {
          ...
          setBulkActionsErrorMessage(`${participantIds?.length} Records Successfully Updated`)
        }
      } else {
        setDisplayTransactionStatusBulkActionError(true)
      ...
      setbulkActionsSuccessIcon(false)
      setBulkActionsErrorTitle('ERROR: Bulk Transaction Status Update')
      setBulkActionsErrorMessage('No transaction history found for the selected participant(s)')
    }
  }
  const [bulkActionSelectedValue, setBulkActionSelectedValue] = useState('Bulk Actions')

  return (
    <div className="flex flex-col justify-between relative h-screen">
      <div className="top-0 sticky  bg-white shadow-md w-full">
      ...
      <div className="bottom-0 sticky absolute flex flex-row px-8 py-2 justify-between m-0  bg-[white] left-0 items-center h-[67px] w-full shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px]">
