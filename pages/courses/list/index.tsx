...
function index() {
    ...
    filters.permanent?.push(
      {
        ...
        value:
          ...
          new Date(new Date(AllFilterData.course_date.from?.setUTCHours(0, 0, 0, 0)).getTime() + 24 * 60 * 60 * 1000)
            .toISOString()
            .replace('T', ' ')
            .slice(0, -5) + '+00'
      },
      {
        ...
        value:
          ...
          new Date(new Date(AllFilterData.course_date.to?.setUTCHours(23, 59, 0, 0)).getTime() + 24 * 60 * 60 * 1000)
            ?.toISOString()
            .replace('T', ' ')
            .slice(0, -5) + '+00'
      }
  ...
  const {
    ...
    meta: {
      select:
        "*,program_teachers(users(contact_id(full_name))) , program_organizers(users(contact_id(full_name))) , program_fee_level_settings(is_custom_fee) , status_id(id,value) ,program_accounting_status_id(id,value)",
    },
    filters: filters,
    sorters: {
      permanent: [
   ...
   */
  const filteredIds = FilterProgramData?.data?.data.map(item => item.id) || []
   ...
   */
  const rowCount: number = Object.values(rowSelection).filter(value => value === true).length
      ...
      <div className="bottom-0 sticky absolute flex flex-row px-8 justify-between m-0 z-[100] bg-[white] left-0 items-center h-[67px] w-full shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px]">
        <div className="flex flex-row items-center gap-2">
          <div className="flex flex-row items-center gap-2">
            ...
            />
            <div>Select All</div>
            <div className="font-semibold">{FilterProgramData?.data?.total || 0}</div>
          </div>
          <div>|</div>
          <div className="flex flex-row gap-2">
            Selected: {allSelected ? FilterProgramData?.data?.total : rowCount} Out of{' '}
            <div className="font-semibold">{FilterProgramData?.data?.total || 0}</div>{' '}
          </div>
        </div>
        <div>
    ...
    <div className="flex flex-row items-center justify-between">
      ...
      <div>
        ...
        <Dialog open={open}>
              ...
              {courseDate ? (
                <div className="flex justify-between items-center w-full">
                  <div className="flex flex-row gap-2 text-[14px]">
                    ...
                    <Text className="font-semibold">{courseDate.from && format(courseDate.from, 'MM/dd/yyyy')}</Text>{' '}
                    {courseDate.to && <span>-</span>}{' '}
                    <Text className="font-semibold">{courseDate.to && format(courseDate.to, 'MM/dd/yyyy')}</Text>
                  </div>
                  <div
                    onClick={() => {
          ...
          <DialogContent closeIcon={false} className="!w-[810px] !h-[446px] bg-[#FFFFFF] !rounded-3xl">
