...
const Footer = ({ stepTitles }: any) => {
  ...
  const { setViewPreviewPage, setNewCourseData, newCourseData } = newCourseStore()
  const { handleClickNext, handleClickPrevious } = useValidateCurrentStepFields()
  ...
  return (
    <div className="flex self-end justify-center w-full gap-4 mt-2">
      ...
      {currentStep == CONTACT_INFO_STEP_NUMBER && (
        <Button className="bg-[#7677F4] w-[117px] h-[46px] rounded-[12px] " onClick={handleClickReviewDetailsButton}>
          Review Details
        </Button>
      )}
    </div>
