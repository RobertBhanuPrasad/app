...
import ViewParticipantLayout from '@components/participant/viewParticipant/layout/ViewParticipantLayout'
...
function index() {
  ...
  const { ParticpantFiltersData, setSelectedTableRows, setSelectedRowObjects } = ParticipantStore()
  const filters: {
    /**
    ...
    filters.permanent.push(
      {
        ...
        operator: 'gte',
        value:
          new Date(ParticpantFiltersData?.registration_date.from?.setHours(0, 0, 0, 0))
            ?.toISOString()
            .replace('T', ' ')
            .slice(0, -5) + '+00'
      },
      {
        ...
        operator: 'lte',
        value:
          new Date(ParticpantFiltersData?.registration_date.to?.setHours(23, 59, 0, 0))
            ?.toISOString()
            .replace('T', ' ')
            .slice(0, -5) + '+00'
      }
    ...
    filters.permanent.push(
      {
        ...
        operator: 'gte',
        value:
          new Date(ParticpantFiltersData?.advanceFilter?.registration_date_range?.from?.setHours(0, 0, 0, 0))
            ?.toISOString()
            .replace('T', ' ')
            .slice(0, -5) + '+00'
      },
      {
        ...
        operator: 'lte',
        value:
          new Date(ParticpantFiltersData?.advanceFilter?.registration_date_range?.to?.setHours(23, 59, 0, 0))
            ?.toISOString()
            .replace('T', ' ')
            .slice(0, -5) + '+00'
      }
  ...
  if (ParticpantFiltersData?.advanceFilter?.health_consent_status?.completed === true) {
    filters.permanent.push({
      field: 'is_health_declaration_checked',
      operator: 'eq',
      value: ParticpantFiltersData?.advanceFilter?.health_consent_status?.completed
    })
  }

  if (ParticpantFiltersData?.advanceFilter?.health_consent_status?.pending === true) {
    filters.permanent.push({
      field: 'is_health_declaration_checked',
      operator: 'eq',
      value: !ParticpantFiltersData?.advanceFilter?.health_consent_status?.pending
    })
  }

  if (ParticpantFiltersData?.advanceFilter?.program_agreement_status?.pending === true) {
    filters.permanent.push({
      field: 'is_program_agreement_checked',
      operator: 'eq',
      value: !ParticpantFiltersData?.advanceFilter?.program_agreement_status?.pending
    })
  }

  if (ParticpantFiltersData?.advanceFilter?.program_agreement_status?.completed === true) {
    filters.permanent.push({
      field: 'is_health_declaration_checked',
      operator: 'eq',
      value: ParticpantFiltersData?.advanceFilter?.program_agreement_status?.completed
    })
  }
        ...
        <div className="bottom-0 sticky absolute flex flex-row px-8 justify-between m-0 z-[100] bg-[white] left-0 items-center h-[67px] w-full shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px]">
          <div className="flex flex-row items-center gap-2">
            ...
            <div className="flex flex-row gap-2">
              Selected: {allSelected ? participantData?.data?.total : rowCount} Out of{' '}
              <div className="font-semibold">{participantData?.data?.total || 0}</div>{' '}
            </div>
          </div>
          <div>
...
const HeaderSection = () => {
  const { ParticpantFiltersData, setParticpantFiltersData, selectedTableRows, selectedRowObjects } = ParticipantStore()
  ...
  const transactionStatusOptions = getOptionValuesByOptionLabel(PARTICIPANT_PAYMENT_STATUS)?.[0]?.option_values
  ...
  const attendanceOptions = getOptionValuesByOptionLabel(PARTICIPANT_ATTENDANCE_STATUS)?.[0]?.option_values

  const paymentStatusOptions = getOptionValuesByOptionLabel(PARTICIPANT_PAYMENT_STATUS)?.[0]?.option_values
  ...
  const handleBulkUpdateTransactionStatus = async (transaction_status_id: number) => {
    const programID: number | undefined = router?.query?.program_id
      ? parseInt(router.query.program_id as string)
      : undefined
    ...
    const transactionPendingStatusID = paymentStatusOptions.find((record: any) => record.value == 'Pending')?.id
            ...
            <div className="flex flex-col gap-4 max-h-[300px] w-full overflow-y-auto scrollbar text-[#333333]">
              {bulkActions == 'attendance'
                ? attendanceOptions?.map((record: any) => (
                    <DropdownMenuItem key={record.id} onClick={() => handleUpdateAttendanceStatus(record.id)}>
                      {record.value}
                    </DropdownMenuItem>
                  ))
                : paymentStatusOptions?.map((record: any) => (
                    <DropdownMenuItem key={record.id} onClick={() => handleBulkUpdateTransactionStatus(record.id)}>
                      {record.value}
                    </DropdownMenuItem>
                  ))}
