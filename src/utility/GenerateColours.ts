export const generateColors = (baseColor: string, numColors: number) => {
  ...
  for (let i = 1; i < numColors; i++) {
    // Start from 1 as we already included the base color
    ...
    const rgbToHex = (rgb: any) =>
      "#" +
      rgb.map((c: any) => Math.round(c).toString(16).padStart(2, "0")).join("");
...
export const getColorWithDecreasedOpacity = (
  baseColor: any,
  position: any,
  maxPosition: any
) => {
  ...
  const hexToRgb = (hex: any) =>
    hex.match(/\w\w/g).map((x: any) => parseInt(x, 16));
