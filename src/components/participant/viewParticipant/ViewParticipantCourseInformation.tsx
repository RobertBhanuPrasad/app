...
function ViewParticipantCourseInformation({ participantId }: any) {
  ...
  const teacherFullNames =
    participantCourseData?.data?.program_id?.program_teachers
      ?.map(
        (teacher: { user_id: { contact_id: { full_name: any } } }) =>
          teacher.user_id.contact_id.full_name
      )
      .join(", ");
  ...
  const coursePaticipantInformation = [
    {
      key: "Course Type",
      value: participantCourseData?.data?.program_id?.program_type_id?.name,
    },
    {
      key: "CourseName",
      value: participantCourseData?.data?.program_id?.program_alias_name_id
        ?.alias_name
        ? participantCourseData?.data?.program_id?.program_alias_name_id
            ?.alias_name
        : "-",
    },
    { key: "Teachers", value: teacherFullNames },
    {
      ...
      value:
        participantCourseData?.data?.participant_attendence_status_id?.value,
    },
    {
      key: "Discount Amount",
      value: participantCourseData?.data?.discounted_amount,
    },
  ];

  return (
    <div>
      <p className="text-[#7677F4] text-[18px] font-[600]">
        Course Information{" "}
      </p>
