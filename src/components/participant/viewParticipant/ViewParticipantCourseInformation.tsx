...
function ViewParticipantCourseInformation({ participantId }: any) {
  ...
  const teacherFullNames = participantCourseData?.data?.program_id?.program_teachers?.map((teacher: { user_id: { contact_id: { full_name: any } } }) => teacher.user_id.contact_id.full_name).join(', ')
