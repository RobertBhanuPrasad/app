...
function ViewParticipantEmailDeliveryLogs({ participantId }: any) {
  ...
  const {
    data: participantEmailDeliveryLogsData,
    isLoading,
    isError,
  } = useOne(query);
    ...
    <div>
      ...
      <div>
        <BaseTable
          ...
          total={
            participantEmailDeliveryLogsData?.data?.email_delivery_logs_section
              ?.length || 0
          }
          pagination={false}
          tableStyles={{
          ...
          data={
            participantEmailDeliveryLogsData?.data
              ?.email_delivery_logs_section || []
          }
          columnPinning={false}
        />
      </div>
