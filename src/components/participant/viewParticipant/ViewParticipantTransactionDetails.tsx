        ...
        <div>
            ...
            <div className="w-full rounded-[10px] !mt-0">
                ...
                <BaseTable
                    ...
                    tableStyles={{
                        table: " !rounded-3xl rounded-md !w-full ", // Custom table styles
                        tableContainer: "",
...
const columns = () => {
    ...
    const columns: ColumnDef<ParticipantPaymentHistoryDataBaseType>[] = [
            ...
            header: () => {
                return (
                    <TableHeader className="  min-w-[100px]">
                        {t(
                            "course.participants:find_participant.transaction_id"
                        )}
                    </TableHeader>
                );
            },
        ...
        },
        {
            ...
            cell: ({ row }) => {
                return (
                    <Text>{formatDateAndTime(row?.original?.created_at)}</Text>
                );
            },
        },
        {
            ...
            cell: ({ row }) => {
                const participantPendingPaymentStatusId =
                ...
                return (
                    <Text
                        className={`  min-w-[150px] ${
                            row?.original?.transaction_status_id ==
                            participantPendingPaymentStatusId
                                ? "text-[#FF6D6D]"
                                : "text-[#7677F4]"
                        }`}
                    >
                        {translatedText(
                            row?.original?.transaction_type_id?.name as object
                        )}
                    </Text>
                );
            },
            ...
            cell: ({ row }) => {
                return (
                    <Text className="lowercase">
                        {translatedText(
                            row?.original?.payment_method_id?.name as object
                        )}
                    </Text>
                );
            },
            ...
            header: () => {
                return (
                    <TableHeader className=" min-w-[170px]">
                        {t(
                            "course.view_course:revenue_summary_tab.organization_fee"
                        )}{" "}
                        (EUR)
                    </TableHeader>
                );
        ...
        },
        {
            ...
            header: () => {
                return (
                    <TableHeader className=" min-w-[150px]">
                        {t(
                            "course.view_course:revenue_summary_tab.expense_fee"
                        )}{" "}
                        (EUR)
                    </TableHeader>
                );
        ...
        },
        {
            ...
            cell: ({ row }) => {
                const organizationFee = row?.original?.organization_fee
                    ? row?.original?.organization_fee
                    : 0;
                const accomdationFee = row?.original?.accommodation_fee
                    ? row?.original?.accommodation_fee
                    : 0;
                const tax = row?.original?.tax ? row?.original?.tax : 0;
                const amountPaidByParticipant =
                    organizationFee + accomdationFee + tax;
                const totalAmount = row?.original?.total_amount
                    ? row?.original?.total_amount
                    : 0;
                const discount = (
                    amountPaidByParticipant - totalAmount
                ).toFixed(2);
        ...
        },
        {
            ...
            header: () => {
                return (
                    <TableHeader className=" min-w-[170px]">
                        {t(
                            "course.participants:view_participant.accommodation_type"
                        )}
                    </TableHeader>
                );
            },
            ...
            cell: ({ row }) => {
                return (
                    <Text>
                        {translatedText(
                            row?.original?.accommodation_type_id
                                ?.accommodation_type_id?.name as object
                        )}
                    </Text>
                );
            },
            ...
            header: () => {
                return (
                    <TableHeader className=" min-w-[200px]">
                        {t(
                            "course.participants:edit_participant.participants_information_tab.accommodation_fee"
                        )}{" "}
                        (EUR)
                    </TableHeader>
                );
            ...
            header: () => {
                return (
                    <TableHeader className="min-w-[120px]">
                        {t("course.view_course:revenue_summary_tab.total_fee")}{" "}
                        (EUR)
                    </TableHeader>
                );
            },
        ...
        },
        {
            ...
            cell: ({ row }) => {
                return (
                    <Text className="lowercase">
                        {translatedText(
                            row?.original?.transaction_fee_level_id
                                ?.name as object
                        )}
                    </Text>
                );
        ...
        },
        {
            ...
            cell: ({ row }) => {
                return (
                    <Text className="lowercase">
                        {translatedText(
                            row?.original?.transaction_status_id?.name as object
                        )}
                    </Text>
                );
            },
                        ...
                        >
                            ...
                            <DropdownMenuContent align="end" asChild={true}>
                                <div className="p-[10px]">
                                    ...
                                    >
                                        {t(
                                            "course.participants:edit_participant.participants_information_tab.edit"
                                        )}
                                    </div>
                                    <div
                                        onClick={() => {
                                    ...
                                    >
                                        {t(
                                            "course.participants:edit_participant.participants_information_tab.view"
                                        )}
                                    </div>
                                    <div
                                        onClick={() => {
                                    ...
                                    >
                                        <div className="p-[5px] hover:bg-[#7677F4]/[0.1] rounded-sm cursor-pointer">
                                            {t(
                                                "course.participants:edit_participant.participants_information_tab.refund"
                                            )}
                                        </div>
                                    </div>
                                    <div
