...
function ViewParticipantTransactionDetails({ participantId }: any) {
  ...
  let {
    ...
    meta: {
      select:
        '*,accommodation_type_id(*,accommodation_type_id(*))' // Selecting specific fields
    },
    filters: {
      permanent: [
        {
...
const columns = () => {
  ...
  const columns: ColumnDef<ParticipantPaymentHistoryDataBaseType>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return <Text className="lowercase">{translatedText(row?.original?.payment_method_id?.name as object)}</Text>
      }
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return (
          <Text>{translatedText(row?.original?.accommodation_type_id?.accommodation_type_id?.name as object)}</Text>
        )
      }
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return (
          <Text className="lowercase">{translatedText(row?.original?.transaction_fee_level_id?.name as object)}</Text>
        )
      }
    },
    {
      ...
      cell: ({ row }) => {
        return <Text className="lowercase">{translatedText(row?.original?.transaction_status_id?.name as object)}</Text>
      }
    },
    {
                ...
                <div className="p-[10px]">
                  ...
                  >
                    <div className="p-[5px] hover:bg-[#7677F4]/[0.1] rounded-sm cursor-pointer">
                      {t('course.participants:edit_participant.participants_information_tab.refund')}
                    </div>
                  </div>
                  <div
                    onClick={() => {
