...
export const handleEditParticipantValues=async(participantId:number)=>{
    ...
    .select("id,transaction_id,payment_transaction_id, transaction_fee_level_id!inner(value),expense_fee,currency_code,accommodation_type_id,accommodation_fee,total_amount,transaction_status_id!inner(id,value),error_message,response_message,payment_method_id,payment_date,send_payment_confirmation,transaction_status,participant_id!inner(id,memo,created_at,roommate_preferences_1,roommate_preferences_2,roommate_preferences_3,roommate_snore,accommodation_snore,participant_code,participant_attendence_status_id,discount_code, organisation_id!inner(name),donation_type!inner(value),donation_date,payment_method,transaction_type!inner(value),contact_id!inner(id,full_name,email,mobile,identification_num,identification_type_id,date_of_birth,street_address,state_id!inner(name),city_id!inner(name),country_id!inner(name)),program_id!inner(id,program_type_id!inner(is_online_program)))")
...
export const getDefaultValues = async (data: ParticipantPaymentHistoryDataBaseType) => {
  ...
  if (typeof data.participant_id === 'object' && data.participant_id !== null) {
    ...
    if (data.transaction_fee_level_id) defaultValues.transaction_fee_level_id = data.transaction_fee_level_id.value

    // program_id
    if (data.participant_id.program_id) defaultValues.program_id = data.participant_id.program_id

    // created_at
    if (data.participant_id?.created_at) defaultValues.created_at = data.participant_id.created_at
    ...
    //    donation_date
    if (data?.participant_id?.donation_date) defaultValues.donation_date = data?.participant_id?.donation_date
    // transaction_type
  ...
  // currency_code
  if (data.currency_code) defaultValues.currency_code = data.currency_code

  // total_amount
  if (data.total_amount) defaultValues.total_amount = data.total_amount

  // accommodation_type_id
  if (data.accommodation_type_id) defaultValues.accommodation_type_id = data.accommodation_type_id

  // accommodation_fee
  if (data.accommodation_fee) defaultValues.accommodation_fee = data.accommodation_fee
  ...
  // payment_method_id
  if (data.payment_method) defaultValues.payment_method = data.payment_method_id?.value;

  // send_payment_confirmation
  if (data.send_payment_confirmation) defaultValues.send_payment_confirmation = data.send_payment_confirmation

  // payment_transaction_id
  if (data.payment_transaction_id) defaultValues.payment_transaction_id = data.payment_transaction_id
  // transaction_id
  if (data.transaction_id) defaultValues.transaction_id = data.transaction_id
  // response_message
  if (data.response_message) defaultValues.response_message = data.response_message

  // error_message
  if (data.error_message) defaultValues.error_message = data.error_message

  // expense_fee
  if (data.expense_fee) defaultValues.expense_fee = data.expense_fee
