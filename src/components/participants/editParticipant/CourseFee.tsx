...
export default function CourseFee() {
  ...
  const { data } = useList({
    ...
    meta: {
      select:
        "id,transaction_fee_level_id(value),total_amount,accommodation_fee,currency_code,participant_id(id,created_at,program_id(id,program_type_id!inner(is_online_program)))",
    },
    filters: [
      {
  ...
  return (
    <div className="" id="Course">
      <Text className="font-semibold text-[18px] pt-[25px]">Course Fees</Text>
      ...
      <div className="flex py-[20px]">
        <div className="w-[303px]">
          ...
          <Text className="font-semibold text-[16px]">
            {courseFeedata?.participant_id?.created_at
              ? formatDateString(
                  new Date(courseFeedata?.participant_id?.created_at)
                )
              : "-"}
          </Text>
        </div>
        ...
        <div className="w-[303px]">
          ...
          <Text className="font-semibold text-[16px]">
            {courseFeedata?.currency_code ? courseFeedata?.currency_code : ""}{" "}
            {courseFeedata?.total_amount
              ? courseFeedata?.participant_id?.program_id?.program_type_id
                  ?.is_online_program
                ? courseFeedata?.total_amount - courseFeedata.accommodation_fee
                : courseFeedata?.total_amount
              : "-"}
          </Text>
        </div>
