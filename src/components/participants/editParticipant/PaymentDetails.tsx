...
export default function PaymentDetails() {
    ...
    const {
        field: { value: discount_code, onChange: discountOnchange },
        fieldState: { error: discountError },
    } = useController({
        name: "discount_code",
    });
    ...
    const paymentData = useList({
        ...
        meta: {
            select: "id,transaction_fee_level_id(value),total_amount,accommodation_fee,currency_code,organization_fee,participant_id(discount_code)",
        },
        filters: [
            {
    ...
    const manualRegistration = false;
    const [discountEnable, setDiscountEnable] = useState(
        manualRegistration
            ? paymentDetailData?.participant_id?.discount_code
                ? true
                : false
            : true
    );
    const applyDiscountCode = () => {
        // need to write the validation for discount code
        ...
        <div className="flex-row pb-[5px]" id="Payment">
            <div className="flex ">
                <div className="w-[303px]">
                    <Text className="text-[#999999]  text-[14px] ">
                        {t(
                            "course.participants:edit_participant.participants_information_tab.course_fee"
                        )}
                    </Text>
                    <Text className="text-[16px] font-semibold">
                        {paymentDetailData?.organization_fee
                            ? `${
                                  paymentDetailData?.currency_code
                                      ? paymentDetailData?.currency_code
                                      : ""
                              } ${paymentDetailData?.organization_fee?.toFixed(
                                  2
                              )}`
                            : "-"}
                    </Text>
                        ...
                        {paymentDetailData?.accommodation_fee
                            ? `${
                                  paymentDetailData?.currency_code
                                      ? paymentDetailData?.currency_code
                                      : ""
                              } ${paymentDetailData?.accommodation_fee?.toFixed(
                                  2
                              )}`
                            : "-"}
                    </Text>
                ...
                <div className="w-[303px]">
                    <Text className="text-[#999999]  text-[14px] ">
                        {t(
                            "course.participants:edit_participant.participants_information_tab.total_fee"
                        )}
                    </Text>
                    <Text className="text-[16px] font-semibold">
                        {paymentDetailData?.total_amount
                            ? `${
                                  paymentDetailData?.currency_code
                                      ? paymentDetailData?.currency_code
                                      : ""
                              } ${paymentDetailData?.total_amount?.toFixed(2)}`
                            : "-"}
                    </Text>
                </div>
            </div>
            <div className="flex py-[10px] gap-8">
                <div className="">
                    <Text className="text-[#999999]  text-[14px] ">
                        {t(
                            "course.participants:edit_participant.participants_information_tab.enter_special_code"
                        )}
                    </Text>
                    ...
                    <div className="flex gap-4">
                        <div>
                            <Input
                                value={discount_code}
                                className={`w-[185px] !h-[40px] resize-none font-semibold rounded-xl border-[#E1E1E1] ${
                                    discountEnable && "cursor-not-allowed"
                                }
                                  `}
                ...
                <div className="w-[305px]">
                    <div className="flex gap-2">
                        <div>
                            <Text className="text-[#999999] text-[14px]  ">
                                {t(
                                    "course.participants:edit_participant.participants_information_tab.attendance_status"
                                )}
                            </Text>
                        </div>
                        <div>
