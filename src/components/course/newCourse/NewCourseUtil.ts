...
=======
import _ from "lodash";
>>>>>>> 506fe59ec15fa55e1e69f5ce2a6ea4cfee8a0edc
import {
...
export const handlePostProgramData = async (body: any, loggedInUserId: number, setProgramId: (by: number) => void) => {
...
>>>>>>> 506fe59ec15fa55e1e69f5ce2a6ea4cfee8a0edc
  ...
  if (body[NewCourseStep1FormNames.program_created_by]) {
    programBody.program_created_by = body[NewCourseStep1FormNames.program_created_by]
  }
  ...
  if (body[NewCourseStep1FormNames.is_registration_via_3rd_party]) {
    programBody.is_registration_via_3rd_party = body[NewCourseStep1FormNames.is_registration_via_3rd_party]
  }

  if (body[NewCourseStep1FormNames.registration_via_3rd_party_url] != undefined) {
    programBody.registration_via_3rd_party_url = body[NewCourseStep1FormNames.registration_via_3rd_party_url]
  }
  ...
  if (body[NewCourseStep2FormNames.program_alias_name_id]) {
    programBody.program_alias_name_id = body[NewCourseStep2FormNames.program_alias_name_id]
  }
  ...
  if (body[NewCourseStep2FormNames.is_geo_restriction_applicable] != undefined) {
    programBody.is_geo_restriction_applicable = body[NewCourseStep2FormNames.is_geo_restriction_applicable]
  }

  // is_registration_required
  if (body[NewCourseStep2FormNames.is_registration_required] != undefined) {
    programBody.is_registration_required = body[NewCourseStep2FormNames.is_registration_required]
  }

  //allowed_countries
  if (body[NewCourseStep2FormNames.allowed_countries]) {
    programBody.allowed_countries = body[NewCourseStep2FormNames.allowed_countries]
  }
  ...
  if (body[NewCourseStep4FormNames.is_early_bird_enabled]) {
    programBody.is_early_bird_enabled = body[NewCourseStep4FormNames.is_early_bird_enabled]
  } else {
  ...
  const { data: feeData, error } = await supabaseClient.functions.invoke('course-fee', {
    method: 'POST',
    body: {
      state_id: stateId,
      city_id: cityId,
  ...
  if (body[NewCourseStep5FormNames.is_residential_program] !== undefined) {
    programBody.is_residential_program = body[NewCourseStep5FormNames.is_residential_program]
  }
  ...
  if (
    ...
    programBody.accommodation_fee_payment_mode = body[NewCourseStep5FormNames.accommodation_fee_payment_mode]
  }

  // step 6
  //bcc_registration_confirmation_email
  if (body[NewCourseStep6FormNames.bcc_registration_confirmation_email]) {
    programBody.bcc_registration_confirmation_email = body[NewCourseStep6FormNames.bcc_registration_confirmation_email]
  }

  console.log('body to create program', programBody)

  const { data: programData, error: programError } = await supabaseClient.from('program').upsert(programBody).select()
...
>>>>>>> 506fe59ec15fa55e1e69f5ce2a6ea4cfee8a0edc
  ...
  if (!(await handlePostProgramAssistantTeachersData(body, programId))) return false

  if (!(await handlePostProgramLanguagesData(body, programId))) return false

  if (!(await handlePostProgramTranslationLanguagesData(body, programId))) return false
  ...
  if (!(await handlePostProgramContactDetailsData(body, programId))) return false
...
export const handlePostProgramInfoData = async (body: any, programId: number) => {
  ...
  const { data: programDetailsInfoResponse, error: programDetailsError } = await supabaseClient
    .from('program_details_info')
    .insert(programDetailsInfoData)
    .select()

  if (programDetailsError) {
    console.log('error while creating data of program details info', programDetailsError)
    return
  } else {
    console.log('program detials info data is created', programDetailsInfoResponse)
  }
}

export const handlePostProgramOrganizersData = async (body: any, programId: number) => {
  ...
  const existingOrganizers: number[] = existingOrganizersResponse?.map(organizer => organizer.user_id) as number[]

  // Step 2: Determine organizers to remove
  const organizersToRemove = _.difference(existingOrganizers, body.organizer_ids)
  ...
  if (organizersToAdd.length > 0) {
    const organizersToAddData: ProgramOrganizersDataBaseType[] = organizersToAdd.map(userId => ({
      program_id: programId,
      user_id: userId
    }))

    const { data: programOrganizerData, error: programOrganizerError } = await supabaseClient
      .from('program_organizers')
    ...
    if (programOrganizerError) {
      console.log('error while creating data of program organizers', programOrganizerError)
      return false
    }
  }
  return true
...
export const handlePostProgramTeachersData = async (body: any, programId: number) => {
  ...
  const existingTeachers: number[] = existingTeachersResponse?.map(teacher => teacher.user_id) as number[]

  // Step 2: Determine teachers to remove
  const teachersToRemove = _.difference(existingTeachers, body[NewCourseStep2FormNames.teacher_ids])

  // Step 3: Remove teachers from the program_teachers table
  if (teachersToRemove.length > 0) {
    await supabaseClient.from('program_teachers').delete().eq('program_id', programId).in('user_id', teachersToRemove)
  }

  // Step 4: Determine teachers to add
  const teachersToAdd = _.difference(body[NewCourseStep2FormNames.teacher_ids], existingTeachers)

  // Step 5: Insert new teachers into the program_teachers table
  if (teachersToAdd.length > 0) {
    const teachersToAddData: ProgramTeachersDataBaseType[] = teachersToAdd.map((userId: number) => ({
      program_id: programId,
      user_id: userId
    }))

    const { data: programTeacherData, error: programTeacherError } = await supabaseClient
      .from('program_teachers')
    ...
    if (programTeacherError) {
      console.log('error while creating data of program teachers', programTeacherError)
      return false
    }
  }
...
export const handlePostProgramAssistantTeachersData = async (body: any, programId: number) => {
  ...
  const existingAssistantTeachers: number[] = existingTeachersResponse?.map(teacher => teacher.user_id) as number[]
  ...
  if (assistantTeachersToAdd.length > 0) {
    const assistantTeachersToAddData: ProgramAssistantTeachersDataBaseType[] = assistantTeachersToAdd.map(
      (userId: number) => ({
        program_id: programId,
        user_id: userId
      })
    ...
    const { data: programAssistantTeacherData, error: programAssistantTeacherError } = await supabaseClient
      .from('program_assistant_teachers')
      .insert(assistantTeachersToAddData)
      .select()

    console.log('program assistant teachers data is created', programAssistantTeacherData)

    if (programAssistantTeacherError) {
      console.log('error while creating data of program assistant teachers', programAssistantTeacherError)
      return false
    }
  }
...
export const handlePostProgramLanguagesData = async (body: any, programId: number) => {
  ...
  const existingLanguages: number[] = existingLanguagesResponse?.map(language => language.language_id) as number[]

  // Step 2: Determine languages to remove
  const languagesToRemove = _.difference(existingLanguages, body[NewCourseStep2FormNames.language_ids])
  ...
  const languagesToAdd = _.difference(body[NewCourseStep2FormNames.language_ids], existingLanguages)

  // Step 5: Insert new languages into the program_languages table
  if (languagesToAdd.length > 0) {
    const languagesToAddData: ProgramLanguagesDataBaseType[] = languagesToAdd.map((languageId: number) => ({
      program_id: programId,
      language_id: languageId
    }))

    const { data: programLanguageData, error: programLanguageError } = await supabaseClient
      .from('program_languages')
    ...
    if (programLanguageError) {
      console.log('error while creating data of program languages', programLanguageError)
      return false
    }
  }
...
export const handlePostProgramTranslationLanguagesData = async (body: any, programId: number) => {
  ...
  const existingTranslationLanguages: number[] = existingTranslationLanguagesResponse?.map(
  ...
  if (translationLanguagesToAdd.length > 0) {
    const translationLanguagesToAddData: ProgramTranslationLanguagesDataBaseType[] = translationLanguagesToAdd.map(
      (languageId: number) => ({
        program_id: programId,
        language_id: languageId
      })
    ...
    const { data: programTranslationLanguageData, error: programTranslationLanguageError } = await supabaseClient
      .from('program_translation_languages')
      .insert(translationLanguagesToAddData)
      .select()

    console.log('program translation languages data is created', programTranslationLanguageData)

    if (programTranslationLanguageError) {
      console.log('error while creating data of program translation languages', programTranslationLanguageError)
      return false
    }
  }
...
export const handleProgramSchedulesData = async (body: any, programId: number) => {
  ...
  if (scheduleIdsToDelete.length > 0) {
    const { data } = await supabaseClient.from('program_schedules').delete().in('id', scheduleIdsToDelete).select()

    console.log('deleted schedules because they are not present in database ', data)
  }

<<<<<<< HEAD
  const schedulesData: ProgramSchedulesDataBaseType[] = body[NewCourseStep3FormNames.schedules].map(
  ...
  console.log('schedulesData to create or update program_schedules was', schedulesData)
=======
...
>>>>>>> 506fe59ec15fa55e1e69f5ce2a6ea4cfee8a0edc
  ...
  const { data, error } = await supabaseClient.from('program_schedules').upsert(schedulesData).select()
...
export const handlePostAccommodations = async (body: any, programId: number) => {
  ...
  const accommodationIdsToDelete = _.difference(
    ...
    body[NewCourseStep5FormNames.accommodation]?.map((accommodation: any) => accommodation.id)
  )
  ...
  const accommodationsData: ProgramAccommodationsDataBaseType[] = body[NewCourseStep5FormNames.accommodation]?.map(
...
export const handlePostProgramContactDetailsData = async (body: any, programId: number) => {
  ...
  if (contactDetailsIdsToDelete.length > 0) {
    ...
    console.log('Deleted these contact details records because they are not present in the form:', data)
  }

  // Prepare contact details data
  const contactDetailsData: ProgramContactDetailsDataBaseType[] = body[NewCourseStep6FormNames.contact]?.map(
    (contactData: any) => {
  ...
  console.log('contactDetailsData need to create in databse', contactDetailsData)
...
const handlePostVenueData = async (body: any, loggedInUserId: number) => {
  ...
  const { data, error } = await supabaseClient.from('venue').upsert(venueBody).select()
  ...
  if (deleteVenueIDs && deleteVenueIDs.length > 0) {
    const { data, error } = await supabaseClient.from('venue').delete().in('id', deleteVenueIDs).select()
...
export const handleProgramStatusUpdate = async (programId: number) => {
  const { data, error }: any = await supabaseClient.from('program').select('id,program_type_id(*)').eq('id', programId)

  if (error) {
    console.log('error was happened while fetch data of program at the time of updat status_id', error)
    return false
  }

  const is_approval_required: boolean | null = data[0]?.program_type_id?.is_approval_required
  ...
  if (is_approval_required) {
    ...
    if (error) {
      console.log('error was happened while patching program status_id as pending', error)
      return false
    } else {
    ...
    if (error) {
      console.log('error was happened while patching program status_id as active', error)
      return false
    } else {
...
export const handleProgramFeeLevelSettingsData = async (body: any, programId: number) => {
  ...
  const modifiedProgramFeeLevel = body?.program_fee_level_settings?.map((feeLevel: any, index: number) => {
 ...
 * @param programId
 */
const handleGenerateProgramCode = async (programId: number, loggedInUserId: number) => {
  ...
  if (error) {
    ...
    let programCode = data[0]?.contact_id?.country_id?.abbr || '' + 'C' + programId
