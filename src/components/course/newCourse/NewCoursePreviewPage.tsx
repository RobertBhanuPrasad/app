...
import { I_AM_CO_TEACHING, NATIONAL_ADMIN, NOT_SUBMITTED, SUPER_ADMIN } from 'src/constants/OptionValueOrder'
...
>>>>>>> 16c42cf2a5d62793ae7f9033fec5228f4041549f
  ...
  const venueSessions = () => {
    ...
    return (
      <div className="w-[291px]">
        <p className="text-sm font-normal text-accent-light text-[#999999]">{t('sessions')}</p>
  ...
  if (user_roles.some(role => role.role_id.order === NATIONAL_ADMIN || role.role_id.order === SUPER_ADMIN)) {
    isUserNationAdminOrSuperAdmin = true
  }

  //Checking Weather a fee is editable or not
  const isFeeEditable = isUserNationAdminOrSuperAdmin || courseFeeSettings?.[0]?.is_program_fee_editable ? true : false
  ...
  const feeLevels = isFeeEditable ? newCourseData?.program_fee_level_settings : defaultFeeLevels

  //Requirement: Need to show only enabled fee levels.
  const enabledFeeLevelData = feeLevels?.filter((feeLevel: { is_enable: boolean }) => feeLevel.is_enable === true)
  ...
  const accountingNotSubmittedStatusId =
    getOptionValueObjectByOptionOrder(COURSE_ACCOUNTING_STATUS, NOT_SUBMITTED)?.id ?? 0
  ...
  const handleErrorMessagesInPreviewPageScreen = async (formData: any) => {
    const requiredFieldsForValidation = await getRequiredFieldsForValidation(formData, loginUserData)
    ...
    const errors: any = newCourseZodSchema.pick(requiredFieldsObject).safeParse(formData)
    ...
    <div className="pb-12">
      ...
      <div className="w-full p-6 text-base bg-white shadow-sm max-h-fit rounded-3xl">
        ...
        <section className="w-full pb-8 text-base border-b">
          ...
          <div className="flex items-center">
            <p className="font-semibold text-accent-primary text-[#333333]">{t('basic_details')}</p>
          ...
          <div className="flex flex-wrap gap-x-[50px] gap-y-[24px] mt-2">
            ...
            <div className="w-[291px]">
              <p className="text-sm font-normal text-accent-light text-[#999999]">{t('organization')}</p>
              <abbr
                className="font-semibold no-underline truncate block text-accent-secondary text-[#666666]"
            ...
            <div className="w-[291px]">
              <p className="text-sm font-normal text-accent-light text-[#999999]">{t('program_organizer')}</p>
              <abbr
                className="font-semibold no-underline truncate block text-accent-secondary text-[#666666]"
        ...
        <section className="w-full py-8 text-base border-b">
          ...
          <div className="flex flex-wrap gap-x-[50px] gap-y-[24px] mt-4">
            <div className="w-[291px]">
              <p className="text-sm font-normal text-accent-light text-[#999999]">{t('course_type')}</p>
              <abbr
                className="font-semibold truncate block no-underline text-accent-secondary text-[#666666]"
                title={
              ...
              {errors?.program_type_id && <span className="text-[#FF6D6D] text-[12px]">{errors?.program_type_id}</span>}
            </div>
            <div className="w-[291px]">
              <p className="text-sm font-normal text-accent-light text-[#999999]">{t('new_strings:teacher')}</p>
              <abbr
              ...
              {errors?.teacher_ids && <span className="text-[#FF6D6D] text-[12px]">{errors?.teacher_ids}</span>}
            </div>
            <div className="w-[291px]">
              <p className="text-sm font-normal text-accent-light text-[#999999]">
            ...
            <div className="w-[291px]">
              <p className="text-sm font-normal text-accent-light text-[#999999]">{t('max_capacity')}</p>
              <abbr
                className="font-semibold truncate no-underline text-accent-secondary text-[#666666]"
            ...
            <div className="w-[291px]">
              <p className="text-sm font-normal text-accent-light text-[#999999]">{t('program_visibility')}</p>
              <abbr
                className="font-semibold truncate no-underline text-accent-secondary text-[#666666]"
            ...
            {hasSuperAdminRole && newCourseData?.is_geo_restriction_applicable && (
              <div className="w-[291px]">
                <p className="text-sm font-normal text-accent-light text-[#999999]">
            ...
            {hasSuperAdminRole && (
              <div className="w-[291px]">
                <p className="text-sm font-normal text-accent-light text-[#999999]">{t('is_geo_restriction')}</p>
                <abbr
                  className="font-semibold truncate no-underline text-accent-secondary text-[#666666]"
        ...
        <section className="w-full py-8 text-base border-b">
          {/* title section */}
          <div className="flex items-center  ">
            <p className="font-semibold text-accent-primary text-[#333333]">{t('time_and_venue')}</p>
          ...
          {/* body */}
          {/* // TODO need to do when the form filed is clear */}
          {programTypeData?.data?.is_online_program === true && newCourseData?.program_type_id !== '' ? (
            <div className="flex flex-wrap gap-x-[50px] gap-y-[24px] mt-4">
              <div className="w-[291px]">
                <p className="text-sm font-normal text-accent-light text-[#999999]">
                ...
                {errors?.online_url && <span className="text-[#FF6D6D] text-[12px]">{errors?.online_url}</span>}
              </div>
              <div className="w-[291px]">
                <p className="text-sm font-normal text-accent-light text-[#999999]">
                ...
                {errors?.state_id && <span className="text-[#FF6D6D] text-[12px]">{errors?.state_id}</span>}
              </div>
              <div className="w-[291px]">
                <p className="text-sm font-normal text-accent-light text-[#999999]">{t('city')}</p>
                <abbr
                  className="font-semibold truncate block no-underline text-accent-secondary text-[#666666]"
                ...
                {errors?.city_id && <span className="text-[#FF6D6D] text-[12px]">{errors?.city_id}</span>}
              </div>
              <div className="w-[291px]">
                <p className="text-sm font-normal text-accent-light text-[#999999]">
                ...
                {errors?.center_id && <span className="text-[#FF6D6D] text-[12px]">{errors?.center_id}</span>}
              </div>
            ...
            <div className="flex flex-wrap gap-x-[50px] gap-y-[24px] mt-4">
              <div className="w-[291px]">
                <p className="text-sm font-normal text-accent-light text-[#999999]">{t('venue_address')}</p>
                <abbr
                ...
                {errors.is_existing_venue && (
                  <span className="text-[#FF6D6D] text-[12px]">{errors.is_existing_venue}</span>
                )}
              </div>
              <div className="w-[291px]">
                <p className="text-sm font-normal text-accent-light text-[#999999]">
                ...
                >
                  {timeFormat?.value
                    ? timeFormat?.value.split(' ')[0] + ' ' + timeFormat?.value.split(' ')[1] + 's'
                    : '-'}
                </abbr>
              </div>
        ...
        <section className="w-full py-8 text-base border-b">
          {/* title section */}
          <div className="flex items-center  ">
            <p className="font-semibold text-accent-primary text-[#333333]">{t('new_strings:fees_information')}</p>
      ...
      4.Early bird cut off editable in settings */}
        ...
        <section className="w-full py-8 text-base ">
          {/* title section */}
          <div className="flex items-center  ">
            <p className="font-semibold text-accent-primary text-[#333333]">{t('new_strings:contact_info')}</p>
          ...
          {newCourseData?.contact?.map((data: any) => {
            return (
              <div className="flex flex-wrap gap-x-[50px] gap-y-[24px] pb-4 mt-2 ">
                <div className="w-[291px]">
                  <p className="text-sm font-normal text-accent-light text-[#999999] ">{t('contact_email')}</p>
                  <abbr
                    className="font-semibold truncate block no-underline text-accent-secondary text-[#666666]"
                ...
                <div className="w-[291px]">
                  <p className="text-sm font-normal text-accent-light text-[#999999] ">{t('contact_name')}</p>
                  <abbr
                    className="font-semibold truncate block no-underline text-accent-secondary text-[#666666]"
  ...
  if (feeLevelSettingsData?.is_custom_fee == true) {
    return (
      <div className="w-[291px]">
        ...
        <abbr title={JSON.stringify(feeLevelSettingsData?.total)} className="no-underline">
          <CardValue className="truncate">
            {countryConfigData?.data?.[0]?.default_currency_code} {feeLevelSettingsData?.total}
          </CardValue>
        </abbr>
      </div>
    )
...
const EarlyBirdFees = ({ feeLevelSettingsData }: { feeLevelSettingsData: ProgramFeeLevelSettingsDataBaseType }) => {
  ...
  if (feeLevelSettingsData?.is_custom_fee == true) {
    return (
      <div className="w-[291px]">
        ...
        <abbr title={JSON.stringify(feeLevelSettingsData?.early_bird_total)} className="no-underline">
          <CardValue className="truncate">
            {countryConfigData?.data?.[0]?.default_currency_code} {feeLevelSettingsData?.early_bird_total}
          </CardValue>
        </abbr>
      </div>
    )
  ...
  return (
    <div className="w-[291px]">
      {/* We have the same fee level types for normal fee and the early bird fee, for differentiating we keep the Early Bird for the Early Bird fees  */}
      <abbr title={`Early Bird ${translatedText(feeLevelData?.data?.name)}`} className="no-underline">
        <CardLabel className="truncate">
    ...
    <AlertDialog open={onEditSuccess}>
      <AlertDialogContent className="flex flex-col items-center justify-center h-[309px] w-[414px] !rounded-[24px] !gap-[34px] !p-[24px]">
        ...
        <AlertDialogDescription className="flex flex-col !w-[366px] !h-[71px] !gap-4 text-[#333333] items-center text-center">
          <h2 className="pt-[16px] text-[24px] font-semibold">{t('successfully_updated')}</h2>
          <p className="text-[16px] font-normal">{t('your_changes_have_been_saved_successfully')}</p>
        </AlertDialogDescription>
        <AlertDialogFooter>
          <div className="w-full flex justify-center items-center gap-5">
            <Button
              ...
              onClick={handleClick}
            >
              {onButtonLoading ? <div className="loader !w-[30px]"></div> : t('go_to_course_details')}
            </Button>
          </div>
        </AlertDialogFooter>
      </AlertDialogContent>
