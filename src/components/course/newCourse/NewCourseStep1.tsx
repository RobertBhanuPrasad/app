...
import { NewCourseStep1FormNames, NewCourseStep2FormNames } from 'src/constants/CourseConstants'
...
const RegistrationGateway = () => {
  ...
  return (
    <div className="flex flex-row gap-6 mt-[60px]">
      <div className="text-[14px] font-normal">Registration via 3rd party gateway</div>
      <Switch
        id="registration"
      ...
      {value && (
        <div className="flex gap-1 flex-col -mt-7 ml-8">
          <div className="text-xs font-normal text-[#333333]">Please input the site's URL *</div>
...
const RadioCards = () => {
  ...
  const hasTeacherRole = user_roles && user_roles.some(role => role.role_id.order === TEACHER)
  ...
  return (
    <RadioGroup value={JSON.stringify(value)} onValueChange={handleOnChange}>
            ...
            >
              <div>
                <RadioGroupCheckItem
                  ...
                  className={
                    value === iAmTeachingId ? '!bg-[#7677F4] !border-none ' : '!border-[#D6D7D8] !shadow-none '
                  }
                />
              </div>
              <div className="flex flex-col items-center gap-[16px]  w-full justify-center">
                <Teacher color={` ${value === iAmTeachingId ? '#7677F4' : '#999999'}`} />I am teaching this course
              </div>
            </Card>
          </Label>
        )}
          ...
          >
            ...
            >
              <RadioGroupCheckItem
                ...
                className={
                  value === iAmCoTeachingId ? '!bg-[#7677F4] !border-none ' : '!border-[#D6D7D8] !shadow-none '
                }
              />
              <div className="flex flex-col items-center gap-[16px]  w-full justify-center">
                <Coteacher color={` ${value === iAmCoTeachingId ? '#7677F4' : '#999999'}`} />I am co-teaching this
                course
              </div>
            </Card>
          </Label>
      ...
      {radioError && <span className="text-[#FF6D6D] text-[14px]">{radioError?.message}</span>}
    </RadioGroup>
  )
}
  ...
  return (
    <div className="w-80 h-20">
      <div className="flex gap-1 flex-col">
        ...
        {organizationError && <span className="text-[#FF6D6D] text-[12px]">{organizationError?.message}</span>}
      </div>
    </div>
  )
}
...
const ProgramOrganizerDropDown = () => {
  ...
  const programOrganizationId = getOptionValueObjectByOptionOrder(USER_ROLE, PROGRAM_ORGANIZER)?.id
  ...
  return (
    <div className="w-80 flex gap-1 flex-col">
      ...
      />
      {programOrganizerError && <span className="text-[#FF6D6D] text-[12px]">{programOrganizerError?.message}</span>}
    </div>
  )
}
