...
const NewCourseThankyouPage = () => {
    ...
    const { data, isLoading: isThankyouPageDataIsLoading } = useOne({
        ...
        id: programId,
        meta: {
            select: "program_code,program_type_id,registration_link,details_page_link,status_id,time_zone_id,program_type_id(name),venue_id(*,center_id(name),state_id(name),city_id(name)),program_teachers(users(contact_id(full_name))),program_schedules(start_time,end_time),status_id(id,value)",
        },
    });
