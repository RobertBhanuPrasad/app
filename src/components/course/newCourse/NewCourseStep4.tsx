...
export default function CourseTable() {
  ...
  const fetchFeeData = async () => {
    ...
    if (error) console.log('error while fetching course fee level settings', error)

    if (earlyBirdCutOff == undefined) {
      setEarlyBirdCutOff(data?.[0]?.early_bird_cut_off_period)
    }

    if (showEarlyBirdColumns == undefined && data?.[0]?.is_early_bird_fee_enabled != null) {
    ...
    <>
      {courseFeeSettings == undefined || isLoading ? (
        ...
        <section>
          ...
          <CourseFeeTable courseFeeSettings={courseFeeSettings} organizationData={organizationData?.data} />
        </section>
      )}
    </>
  )
...
function CourseFeeTable({ courseFeeSettings, organizationData }: any) {
  ...
  const taxRate = organizationData?.tax_enabled ? organizationData?.tax_rate / 100 : 0
  ...
  // Data for the table
  const courseFeeData: FeeLevelType[] = courseFeeSettings?.[0]?.program_fee_level_settings?.map((val: any) => {
    ...
    if (courseFeeSettings?.[0]?.is_early_bird_fee_enabled) {
      modifiedFeeLevels = {
        ...modifiedFeeLevels,
        earlyBirdSubTotal: (val?.early_bird_total - val?.early_bird_total * taxRate)?.toFixed(2),
        ...
        cell: ({ row }) => {
          ...
          return (
            <Checkbox
              ...
              disabled={
                row?.original.feeLevelId == regularFeeLevelId && row?.original?.is_custom_fee == false ? true : false
              }
            />
          )
        },
  ...
  return (
    <div className="flex flex-col justify-center w-[70vw]">
      ...
      <div className="h-auto overflow-x-scroll rounded-md border">
        {isFeeEditable ? (
          feeLevels?.length > 0 && <DataTable columns={feeColumns} data={courseFeeData} />
        ) : (
            ...
            <div className="w-80 mt-9">
              ...
              <DateField
                ...
                onChange={(date: any) => {
                  ...
                  const differenceInMilliseconds: number = courseStartDate.getTime() - date.getTime()

                  // Convert milliseconds to days
                  const oneDayInMilliseconds: number = 1000 * 60 * 60 * 24

                  const differenceInDays: number = Math.floor(differenceInMilliseconds / oneDayInMilliseconds)
