...
export function CourseFeeTable({ courseFeeSettings }: any) {
  ...
  const isFeeEditable = isUserNationAdminOrSuperAdmin || courseFeeSettings?.[0]?.is_program_fee_editable

  const {
    field: { value: showEarlyBirdColumns = false, onChange: setShowEarlyBirdColumns }
  ...
  const courseFeeData: FeeLevelType[] = courseFeeSettings?.[0]?.program_fee_level_settings?.map((val: any) => {
    return {
      ...
      feeLevelLabel: val?.is_custom_fee ? val?.custom_fee_label : val?.fee_level_id?.value,
    ...
    },
    {
      cell: ({ row }) => {
        ...
        const handleTotalChange = () => {
          ...
          setValue(`feeLevels[${row?.index}][tax]`, (feeAmount * taxRate) / 100)
        }
  ...
  const editableEarlyBirdFeeColumn: ColumnDef<FeeLevelType>[] = [
    ...
    },
    {
      cell: ({ row }) => {
        ...
        const handleTotalChange = () => {
          onChange(feeAmount || 0)
          setValue(`feeLevels[${row?.index}][earlyBirdSubTotal]`, normalEarlyBirdFee)
          setValue(`feeLevels[${row?.index}][earlyBirdTax]`, (feeAmount * taxRate) / 100)
        }
      ...
      {courseFeeSettings?.[0]?.is_early_bird_fee_enabled && (
        <div className="flex justify-end items-center gap-2 py-4">
          <Checkbox
            ...
            onCheckedChange={val => setShowEarlyBirdColumns((prev: boolean) => !prev)}
            className="w-6 h-6 border-[1px] border-[#D0D5DD] rounded-lg"
          />
