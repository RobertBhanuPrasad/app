...
const removeCourseNameFromColumn = async () => { 
  const FalseAliasName = await useFalseAliasName()
  const courseNameIndex = columns.findIndex(column => column.column_name === 'Course Name');
  if (!FalseAliasName && courseNameIndex !== -1) {
    columns.splice(courseNameIndex, 1);
  }
};
removeCourseNameFromColumn()
