    ...
    <div>
      ...
      <Accordion type="multiple">
        ...
        <AccordionItem
          ...
          className="px-4 mt-6 border rounded-3xl"
        >
      ...
      <div className="flex w-[60%] ml-auto mt-6">
        <div className="flex items-center gap-4">
          <Button className="bg-white border text-primary border-primary">
            Back
          </Button>
    ...
    <div>
      {offlineRevenueData?.isLoading ? (
        ...
        <div>
          ...
          <TableHeader className="mt-[10px]">
            Revenue Details - Participants
          </TableHeader>
          {/* //Todo : Requirement is not clear */}
        </div>
      )}
...
export const offlineRevenueTableColumns: ExtendedColumnDef<any>[] = [
  {
    ...
    cell: ({ row }) => {
      ...
      return <Text className="min-w-[150px]">{date}</Text>;
    },
  },
  {
    accessorKey: "amount",
...
const ExpenseDetailsAccordionContent = ({
  programId,
}: {
  programId: number;
}) => {
  const { tableQueryResult: changeLogData } = useTable({
    ...
    meta: {
      select:
        "*,user_id(contact_id(full_name)), caf_status_id(id,value) ,created_at ,comment",
    },
    pagination: {
      pageSize: 1000,
    },
    ...
    <div>
      {changeLogData?.isLoading ? (
        ...
        <div>
          ...
          />
          <p className="text-xs">
            <span className="font-medium ">Note:</span> Please enter a complete
            and accurate your bank account information above for each person to
            be reimbursed to ensure that they receive their reimbursement.
          </p>
          {/* Reimbursement Summary Total  Calculations */}
          <div className="border flex mt-4 rounded-xl ">
            <div className="flex-[1.5] flex border-r py-2 px-4">
              <TableHeader className="flex-[0.8] font-semibold">
                Total Requested:
              </TableHeader>
              <p className="flex-[0.2]">0.00</p>
            </div>
    ...
    cell: ({ row }) => {
      return (
        <abbr
          ...
          title={row.original.user_id.contact_id.full_name}
        >
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const date = formatDateString(
        new Date(row.original.created_at.slice(0, 10))
      );
      return (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      return (
        <abbr
          className="no-underline"
          title={`${row.original.caf_status_id.value} (${row.original.comment})`}
        >
...
export const reimbursementSummaryColumns: ExtendedColumnDef<any>[] = [
  {
    ...
    header: () => {
      return (
        <TableHeader className="min-w-[300px]">
  ...
  },
  {
    ...
    header: () => {
      return (
        <TableHeader className="min-w-[250px]">
...
export const expenseDetailsColumns: ExtendedColumnDef<any>[] = [
  {
    ...
    header: () => {
      return (
        <TableHeader className="min-w-[150px]">Expense Category</TableHeader>
      );
    },
    cell: ({ row }) => {
      return (
  ...
  },
  {
    ...
    header: () => {
      return (
        <TableHeader className="min-w-[150px]">Amount (EUR) (i)</TableHeader>
      );
    },
    cell: ({ row }) => {
      return (
